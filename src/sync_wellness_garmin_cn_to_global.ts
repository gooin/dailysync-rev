...
const main = async () => {
    try {
        ...
        await syncGarminWellnessRecentDays(clientCN, clientGlobal, undefined, true);
    } catch (e) {
        axios.get(
            `https://api.day.app/${BARK_KEY}/Garmin CN -> Garmin Global Wellness 同步数据运行失败了，快去检查！/${e.message}`);
