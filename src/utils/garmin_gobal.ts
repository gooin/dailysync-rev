...
export const migrateGarminGlobal2GarminCN = async (count = 200) => {
    ...
    for (let j = 0; j < runningActs.length; j++) {
        ...
        // 上传到佳明中国区
        console.log(`本次开始向中国区上传第 ${number2emoji(j + 1)} 条数据，相对总数上传到 ${number2emoji(j + 1 + actIndex)} 条，  【 ${act.activityName} 】，开始于 【 ${act.startTimeLocal} 】，活动ID: 【 ${act.activityId} 】`);
...
export const syncGarminGlobal2GarminCN = async () => {
    ...
    const globalActs = await clientGlobal.getActivities(0, 10);
    ...
    if (latestCnActStartTime === latestGlobalActStartTime) {
        ...
        for (let i = 0; i < globalActs.length; i++) {
            ...
            if (globalAct.startTimeLocal > latestCnActStartTime) {
                ...
                // 上传到佳明中国区的
                console.log(`本次开始向中国区上传第 ${number2emoji(i + 1)} 条数据，【 ${globalAct.activityName} 】，开始于 【 ${globalAct.startTimeLocal} 】，活动ID: 【 ${globalAct.activityId} 】`);
                await uploadGarminActivity(filePath, clientCN);
